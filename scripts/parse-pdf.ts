...
let strict = false
let force = process.env.PDF_PARSE_FORCE === "true"
...
const configuredConcurrency = Number.parseInt(
  process.env.PDF_PARSE_CONCURRENCY || "",
  10
)
let concurrency = Math.max(
  1,
  configuredConcurrency || Math.min(4, availableParallelism)
)
...
for (let i = 0; i < args.length; i++) {
  ...
  if (args[i] === "--concurrency" && args[i + 1]) {
    concurrency = Math.max(1, Number.parseInt(args[i + 1], 10) || concurrency)
  }
}
...
function getParserFingerprint(): string {
  ...
  try {
    ...
    pdfjsVersion = pdfjsPackage.version || pdfjsVersion
  } catch {}

  return crypto
    .createHash("sha256")
    .update(fs.readFileSync(scriptPath))
    .update(`pdfjs-dist@${pdfjsVersion}`)
    .digest("hex")
}

function getParseCachePaths() {
  const relativeOutput = path.relative(process.cwd(), path.resolve(outputDir))
  const cacheKey = relativeOutput.replace(/[^a-zA-Z0-9._-]+/g, "-")
...
function loadParseCacheManifest(
  manifestPath: string
): ParseCacheManifest | null {
  try {
    const parsed = JSON.parse(
      fs.readFileSync(manifestPath, "utf-8")
    ) as ParseCacheManifest
...
function materializeCachedFile(cachePath: string, outputPath: string): void {
  try {
    fs.linkSync(cachePath, outputPath)
  } catch (error) {
    const code = (error as NodeJS.ErrnoException).code
    if (!code || !["EXDEV", "EPERM", "EOPNOTSUPP"].includes(code)) throw error
...
function parseDateFromLine(line: string): string | undefined {
  const dateMatch = line.match(
    /(\d[\d\s]{0,4})\s*年\s*(\d[\d\s]{0,2})\s*月\s*(\d[\d\s]{0,2})\s*日/
  )
  return dateMatch
    ? parseRocDate(dateMatch[1], dateMatch[2], dateMatch[3])
    : undefined
}

function findDeclarationDate(text: string): string | undefined {
  const inlineMatch = text.match(
    /申\s*報\s*日(?!\s*期)\s*(\d[\d\s]{0,4})\s*年\s*(\d[\d\s]{0,2})\s*月\s*(\d[\d\s]{0,2})\s*日/
  )
  if (inlineMatch)
    return parseRocDate(inlineMatch[1], inlineMatch[2], inlineMatch[3])
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const sameLine = sameLineMatch
      ? parseRocDate(sameLineMatch[1], sameLineMatch[2], sameLineMatch[3])
      : undefined
  ...
  const noticeMatch = text.match(
    /通\s*知\s*日\s*期[：:\s]*(\d[\d\s]{0,4})\s*年\s*(\d[\d\s]{0,2})\s*月\s*(\d[\d\s]{0,2})\s*日/
  )
  if (noticeMatch)
    return parseRocDate(noticeMatch[1], noticeMatch[2], noticeMatch[3])
...
function findDeclarationType(text: string): string | undefined {
  const typeRe =
    /(信託財產管理或處分指示通知|新增信託財產申報|信託財產申報|定期申報|就職申報|卸任申報|代理申報|兼任申報|更正申報)/
  const headerType = text.match(
    /公\s*職\s*人\s*員\s*信\s*託\s*財\s*產\s*管\s*理\s*或\s*處\s*分\s*指\s*示\s*通\s*知\s*表/
  )
  if (headerType) return "信託財產管理或處分指示通知"

  const inlineMatch = text.match(/申\s*報\s*類\s*別\s+([^\s\n]+)/)
  if (inlineMatch && typeRe.test(inlineMatch[1]))
    return inlineMatch[1].match(typeRe)![1]
...
const PERSON_NAME_BANNED_RE =
  /服務機關|職稱|申報|姓名|配偶|子女|立法院|有限公司|有限|股份|公司|信託|分行|銀行|證券|基金|股票|能源/
...
function extractLeadingPersonName(line: string): string | undefined {
  ...
  const leadingNameRe = new RegExp(
    `^(${PERSON_NAME_TEXT}{2,30}?)(?=\\s+\\d+[.．]\\s*)`
  )
  const plainNameRe = new RegExp(
    `^(${PERSON_NAME_TEXT}{2,30})(?=\\s+[服職]|\\s*$)`
  )
  const matches = [trimmed.match(leadingNameRe), trimmed.match(plainNameRe)]
...
function findHeaderName(text: string): string | undefined {
  const inlineNameRe = new RegExp(
    `申\\s*報\\s*人(?:\\s*姓\\s*名)?[：:\\s]+(${PERSON_NAME_TEXT}{2,30}?)(?=\\s+[服職]|\\s*\\n|\\s*$)`
  )
    ...
    for (let j = i - 1; j >= 0 && j >= i - 3; j--) {
      for (const offset of [1, 2]) {
        const combined = normalizePersonName(
          `${lines[j].trim()} ${lines[i + offset]?.trim() || ""}`
        )
...
function parseHeader(text: string): Partial<LegislatorDeclaration> {
  ...
  const orgTitleLine = text.match(
    /1\.\s*([^\s]+(?:院|議會|代表會|公所|市政府|縣政府))\s+1\.\s*([^\s]+)/
  )
  if (orgTitleLine) {
  ...
  if (!result.organization) {
    const orgLine = text.match(
      /1\.\s*([^\s]+(?:院|議會|代表會|公所|市政府|縣政府))/
    )
...
const CURRENCY_PATTERN =
  "美元|日圓|歐元|英鎊|新加坡幣|新臺幣|新台幣|港幣|澳幣|南非幣|紐西蘭幣|人民幣"
const STOCK_TAIL_RE = new RegExp(
  `([\\d,]+)\\s+([\\d,.]+)\\s+(?:(${CURRENCY_PATTERN})\\s+)?([\\d,]+(?:\\.\\d+)?)\\s*$`
)
...
function normalizeLeadingTickerName(name: string): string {
  return name.replace(/^([A-Za-z]{2,10})(?=\()/, (ticker) =>
    ticker.toUpperCase()
  )
}
...
function cleanStockName(name: string): string {
  ...
  return normalizeLeadingTickerName(
    cleaned
      .replace(/\s*([「」：，、（）()])\s*/g, "$1")
      .replace(/\s*\/\s*/g, " / ")
      .replace(/\s{2,}/g, " ")
...
function shouldAppendStockSuffix(
  ...
  return /^(有限公司|公司|司|託[」)]?|信託[」)]?|因[:：]?|非國內|上[櫃市]股票|[）)])/.test(
    stripped
  )
}
...
function parseStocks(
  stockText: string
): LegislatorDeclaration["securities"]["stocks"] {
  const empty = {
    totalNTD: 0,
    items: [] as LegislatorDeclaration["securities"]["stocks"]["items"],
  }
  if (!stockText || isBlank(stockText)) return empty

  const stockTotalMatch = stockText.match(
    /股票[（(]總價額[：:]\s*新臺幣\s*([\d,]+)\s*元[）)]/
  )
  ...
  for (let li = 0; li < lines.length; li++) {
    ...
    while (lookahead < lines.length) {
      ...
      if (
        !parsedText ||
        shouldAppendStockSuffix(
          parsedText.name,
          next,
          Boolean(parsedText.owner)
        )
      ) {
...
const TRUSTEE_FULL_RE =
  /中\s*信\s*銀\s*行|台\s*北\s*富\s*邦|國\s*泰\s*證\s*券|國\s*泰\s*綜\s*合\s*證\s*券|國\s*泰\s*世\s*華|元\s*大\s*證\s*券|富\s*邦\s*銀\s*行|富\s*邦\s*證\s*券|永\s*豐\s*證\s*券|永\s*豐\s*金\s*證|保\s*德\s*信\s*證\s*券\s*投\s*資|群\s*益\s*證\s*券\s*投\s*資|摩\s*根\s*證\s*券\s*投\s*資|臺\s*灣\s*銀\s*行|臺\s*銀\s*證\s*券|華\s*南\s*永\s*昌|凱\s*基\s*證\s*券|京\s*城\s*商\s*業|京\s*城\s*證\s*券|臺\s*灣\s*新\s*光|新\s*光\s*證\s*券|上\s*海\s*銀\s*行|玉\s*山\s*銀\s*行|合\s*庫\s*商\s*業\s*銀\s*行|星\s*展\s*台\s*灣/
const TRUSTEE_ONLY_RE =
  /^(中信銀行信託部?|玉山銀行信託部?|合庫商業銀行信託部?|信託部?|部|分公司|母分公司|限公司|有限公司|台北富邦銀行敦北分行|國泰證券忠孝分公司|國泰綜合證券敦南二分公司|元大證券基隆分公司|元大證券基隆|富邦銀行|星展台灣南京東路分行|新光證券)$/
const FUND_TAIL_RE = new RegExp(
  `([\\d,]+\\.?\\d*)\\s+([\\d,]+\\.?\\d*)\\s+(?:(${CURRENCY_PATTERN})\\s+)?([\\d,]+\\.?\\d*)\\s*$`
)

function isFundHeader(line: string): boolean {
  return /^3\.\s*基金|^基金受益|^票\s*面|^名\s*稱|^（單位|^總\s*額$|^新臺幣總額/.test(
    line.trim()
  )
}
...
function isTrusteeLine(line: string): boolean {
  ...
  return (
    TRUSTEE_ONLY_RE.test(stripped) ||
    (withoutLeadingDigits !== stripped &&
      TRUSTEE_ONLY_RE.test(withoutLeadingDigits))
  )
}
...
function extractNameFromMixed(line: string): string | null {
  ...
  if (
    stripped.length > 1 &&
    (stripped.endsWith("部") ||
      stripped.endsWith("分公司") ||
      stripped.endsWith("分行"))
  ) {
    const name = stripped
      .replace(/部$/, "")
      .replace(/分公司$/, "")
...
function isNameSuffix(stripped: string): boolean {
  ...
  if (
    /^(月配|配息|息來源|級別|避險級別|日圓|美元|澳幣|南非幣|新臺幣|新台幣|數據|穩月配|權)/.test(
      stripped
    )
  )
    return true
...
function shouldAppendShortFundSuffix(
  nameSoFar: string,
  fragment: string
): boolean {
  const stripped = fragment.trim().replace(/\s/g, "")
  if (stripped.length === 0 || stripped.length > 2 || /^\d+$/.test(stripped))
    return false
...
function stripFundTrusteeText(text: string): string {
  return (
    text
      ...
      .replace(
        /保\s*德\s*信\s*證\s*券[\s\u4e00-\u9fff]*信\s*託[\s\u4e00-\u9fff]*/g,
        ""
      )
      .replace(
        /群\s*益\s*證\s*券[\s\u4e00-\u9fff]*信\s*託[\s\u4e00-\u9fff]*/g,
        ""
      )
      .replace(
        /摩\s*根\s*證\s*券[\s\u4e00-\u9fff]*信\s*託[\s\u4e00-\u9fff]*/g,
        ""
      )
      ...
      .replace(
        /[\u4e00-\u9fff]*投\s*資\s*信\s*託\s*股\s*份\s*有\s*限\s*公\s*司/g,
        ""
      )
...
function splitFundNameOwner(
  ...
  for (let i = cleanedParts.length - 1; i >= 0; i--) {
    ...
    const ownerOnly = line.replace(/\s/g, "")
    if (isLikelyShortCjkPersonName(ownerOnly)) {
      const name = cleanedParts
        .slice(0, i)
        .concat(cleanedParts.slice(i + 1))
        .join("")
        .replace(/\s/g, "")
      return name ? { name, owner: ownerOnly } : null
    }
...
function parseFunds(
  fundText: string
): LegislatorDeclaration["securities"]["funds"] {
  const empty = {
    totalNTD: 0,
    items: [] as LegislatorDeclaration["securities"]["funds"]["items"],
  }
  ...
  while (i < lines.length) {
    ...
    if (isFundHeader(line) || isFundSkip(line)) {
      i++
      continue
    }
    if (isTrusteeLine(line)) {
      i++
      continue
    }
    ...
    while (i < lines.length) {
      const l = lines[i]
      if (isFundHeader(l) || isFundSkip(l)) {
        i++
        continue
      }
    ...
    let namePrefix = dm
      ? stripFundTrusteeText(dm.prefix).replace(/\s/g, "")
      : ""
    // Remove any remaining trustee fragments
    namePrefix = namePrefix.replace(/-$/, "").trim()
    ...
    while (i < lines.length) {
      ...
      if (np !== null) {
        if (np.length > 0) {
          ...
          if (
            isNameSuffix(npClean) ||
            shouldAppendShortFundSuffix(nameSoFar, npClean)
          ) {
      ...
      const nameSoFar = allNameFrags.join("")
      if (
        !nameSoFar.match(/基金|ETF|股息/) &&
        stripped.match(/基金|ETF|股息/)
      ) {
        allNameFrags.push(stripped)
        i++
        continue
...
function isTrustDeclaration(text: string): boolean {
  return (
    /信\s*託\s*財\s*產\s*申\s*報\s*表/.test(text) ||
    /信\s*託\s*財\s*產\s*管\s*理\s*或\s*處\s*分\s*指\s*示\s*通\s*知\s*表/.test(
      text
    )
  )
}
...
const ROC_DATE_TEXT =
  "\\d[\\d\\s]{0,4}\\s*年\\s*\\d[\\d\\s]{0,2}\\s*月\\s*\\d[\\d\\s]{0,2}\\s*日"
const TRUST_STOCK_INLINE_RE = new RegExp(
  `^(.+?)\\s+([\\d,]+)\\s+([\\u4e00-\\u9fff○]{2,4})\\s+(.+?)\\s+${ROC_DATE_TEXT}\\s+([\\d,.]+)\\s+([\\d,]+(?:\\.\\d+)?)$`
)
const TRUST_STOCK_DATA_RE = new RegExp(
  `^([\\d,]+)\\s+([\\u4e00-\\u9fff○]{2,4})\\s+(.+?)\\s+${ROC_DATE_TEXT}\\s+([\\d,.]+)\\s+([\\d,]+(?:\\.\\d+)?)$`
)
const TRUST_STOCK_OWNER_RE = new RegExp(
  `^([\\u4e00-\\u9fff○]{2,4})\\s+(.+?)\\s+${ROC_DATE_TEXT}$`
)
const TRUST_STOCK_NUMBERS_RE = /^([\d,]+)\s+([\d,.]+)\s+([\d,]+(?:\.\d+)?)$/
const TRUST_STOCK_INSTRUCTION_RE =
  /^(.+?)\s+([\d,]+)\s+([\d,.]+)\s+([\u4e00-\u9fff○]{2,4})(?:\s+.+)?$/
...
function matchTrustStockNumbers(
  line: string
): { shares: number; parValue: number; ntdTotal: number } | null {
...
function shouldAppendTrustStockSuffix(
  nameSoFar: string,
  line: string
): boolean {
  const stripped = line.trim().replace(/\s/g, "")
...
function parseTrustStocks(
  ...
  const stockTotalMatch = trustText.match(
    /股票[（(]總價額[：:]\s*新臺幣\s*([\d,]+)\s*元[）)]/
  )
  const stockTotalNTD = stockTotalMatch ? parseInteger(stockTotalMatch[1]) : 0
  const items: typeof empty.items = []
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (dataOnly && pendingNameParts.length > 0) {
      ...
      while (nextIndex < lines.length) {
        ...
        if (
          matchTrustStockInline(next) ||
          matchTrustStockData(next) ||
          matchTrustStockNumbers(next) ||
          matchTrustStockOwner(next)
        )
          break
...
function parseTrustSecurities(
  text: string
): LegislatorDeclaration["securities"] {
...
async function parseAssetDeclaration(
  text: string
): Promise<LegislatorDeclaration> {
  const sections = splitSections(text)
  const firstSectionIdx = text.search(/[（(]\s*[二三四五六七八九十]+\s*[）)]/)
  const headerText =
    firstSectionIdx > 0 ? text.slice(0, firstSectionIdx) : text.slice(0, 500)
  ...
  return {
    ...
    minorChildren: [],
    securities: trustDeclaration
      ? parseTrustSecurities(trustSecuritiesText)
      : parseSecurities(sections["八"] || ""),
    notes: parseNotes(
      trustDeclaration ? sections["四"] || "" : sections["十三"] || ""
    ),
  }
}
...
function parseChangeHeader(text: string): Partial<ChangeDeclaration> {
  ...
  const fromMatch = text.match(
    /前次申報日期\s*民?國?\s*(\d[\d\s]{0,4})\s*年\s*(\d[\d\s]{0,2})\s*月\s*(\d[\d\s]{0,2})\s*日/
  )
...
const CHANGE_SKIP_RE =
  /名\s*稱|證\s*券\s*交|變\s*動\s*時|變\s*動\s*原|總\s*額|國內上市|監察院公報/
...
function findBrokerIndex(prefix: string): number {
  const namedBrokerRe = new RegExp(
    `(^|[\\s＊*]+)(${NAMED_BROKER_PATTERN})`,
    "g"
  )
...
function splitStockAndBroker(prefix: string): {
  stockName: string
  broker: string
} {
  ...
  return {
    stockName: prefix.replace(/\s/g, "").replace(/[＊*]$/, ""),
    broker: "",
  }
}

function findSplitChangeOwner(
  rawLines: string[],
  index: number,
  beforeNums: string
): { owner: string; prefix: string } | null {
...
function parseChangeStocksInline(
  rawLines: string[]
): NonNullable<ChangeDeclaration["stocks"]> {
  ...
  for (let i = 0; i < rawLines.length; i++) {
    ...
    const datedNumMatch = beforeReason.match(
      /([\d,]+)\s+([\d,.]+(?:\s+\d{1,2})?)\s+(\d[\d\s]{0,4})\s*年\s*(\d[\d\s]{0,2})\s*月\s*(\d[\d\s]{0,2})\s*日\s*$/
    )
    const numMatch = datedNumMatch
      ? null
      : beforeReason.match(/([\d,]+)\s+([\d,.]+(?:\s+\d{1,2})?)\s*$/)
    if (!datedNumMatch && !numMatch) continue

    const shares = parseInteger(datedNumMatch ? datedNumMatch[1] : numMatch![1])
    if (shares === 0) continue
    const changePrice = parseDecimal(
      datedNumMatch ? datedNumMatch[2] : numMatch![2]
    )
    let inlineDateStr = ""
    if (datedNumMatch) {
    ...
    const beforeNums = beforeReason
      .slice(0, datedNumMatch ? datedNumMatch.index! : numMatch!.index!)
      .trim()
    const ownerMatch = beforeNums.match(/([\u4e00-\u9fff○]{2,4})\s*$/)
    const splitOwner = ownerMatch
      ? null
      : findSplitChangeOwner(rawLines, i, beforeNums)
    if (!ownerMatch && !splitOwner) continue

    const owner = ownerMatch ? ownerMatch[1] : splitOwner!.owner
    const prefix = ownerMatch
      ? beforeNums.slice(0, ownerMatch.index!).trim()
      : splitOwner!.prefix
    ...
    let day = 0
    let nextIdx = i + 1
    while (nextIdx < rawLines.length && /^\s*$/.test(rawLines[nextIdx]))
      nextIdx++
...
function parseChangeStocksNumbersFirst(
  rawLines: string[]
): NonNullable<ChangeDeclaration["stocks"]> {
  const items: NonNullable<ChangeDeclaration["stocks"]> = []
  ...
  for (let i = 0; i < rawLines.length; i++) {
    ...
    let infoIdx = i + 1
    while (infoIdx < rawLines.length && /^\s*$/.test(rawLines[infoIdx]))
      infoIdx++
    ...
    const dateMatch = infoLine.match(
    ...
    if (!reason) {
      ...
      if (prevIdx >= 0) {
        ...
        const reasonInPrev = prevStripped.match(
          /(存券匯撥|變更面額|劃撥配發|減資轉入|減資轉出|買進|賣出|配股|轉讓|贈與)/
        )
        if (reasonInPrev) {
          const reasonStart = prevLine.search(
            /存\s*券\s*匯\s*撥|變\s*更\s*面\s*額|劃\s*撥\s*配\s*發|減\s*資\s*轉\s*入|減\s*資\s*轉\s*出|買\s*進|賣\s*出|配\s*股|轉\s*讓|贈\s*與/
          )
          if (reasonStart > 0) {
...
function parseChangeStocks(text: string): ChangeDeclaration["stocks"] {
  ...
  const items =
    numbersFirstResult.length >= inlineResult.length
      ? numbersFirstResult
      : inlineResult
...
async function parseChangeDeclarationDoc(
  text: string
): Promise<ChangeDeclaration> {
  const sections = splitSections(text)
  const firstSectionIdx = text.search(/[（(]\s*[二三四五六七八九十]+\s*[）)]/)
  const headerText =
    firstSectionIdx > 0 ? text.slice(0, firstSectionIdx) : text.slice(0, 500)
...
function imageOnlyFallbackDocument(
  filePath: string
): LegislatorDocument | null {
...
async function parsePDF(filePath: string): Promise<LegislatorDocument[]> {
  ...
  const declHeaderRe =
    /公\s*職\s*人\s*員\s*(變\s*動\s*|信\s*託\s*)?財\s*產\s*申\s*報\s*表/g
  ...
  if (headerPositions.length > 1) {
    ...
    for (let i = 0; i < headerPositions.length; i++) {
      ...
      const end =
        i + 1 < headerPositions.length ? headerPositions[i + 1] : text.length
...
function validateParsedDocument(doc: LegislatorDocument, source: string): void {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(doc.declarationDate))
    errors.push(`invalid declarationDate "${doc.declarationDate}"`)

  if (doc.type === "declaration") {
    for (const item of doc.securities.stocks.items) {
      if (!item.name || /^\d+$/.test(item.name) || /★/.test(item.name))
        errors.push(`invalid stock name "${item.name}"`)
      if (!isLikelyPersonName(item.owner))
        errors.push(`invalid stock owner "${item.owner}"`)
    }
    for (const item of doc.securities.funds.items) {
      if (!item.name || /^\d+$/.test(item.name) || /★/.test(item.name))
        errors.push(`invalid fund name "${item.name}"`)
    ...
    for (const item of doc.stocks || []) {
      if (
        ...
        errors.push(`invalid change stock "${item.name}" / "${item.broker}"`)
      if (!isLikelyPersonName(item.owner))
        errors.push(`invalid change owner "${item.owner}"`)
      if (!/^\d{4}-\d{2}-\d{2}$/.test(item.changeDate))
        errors.push(`invalid changeDate "${item.changeDate}"`)
    }
  }

  if (errors.length > 0) {
    throw new Error(
      `${source}: ${errors.slice(0, 8).join("; ")}${errors.length > 8 ? `; ... ${errors.length - 8} more` : ""}`
    )
  }
}
...
async function main() {
  ...
  // Rebuild the output directory from the persistent parse cache. Cloudflare
  // Pages preserves .next/cache between builds, unlike generated data/ files.
  ...
  const indexName =
    indexNameByOutputDir[path.basename(outputDir)] ?? "index.json"
  ...
  const cachePaths = getParseCachePaths()
  fs.mkdirSync(outputDir, { recursive: true })
  fs.mkdirSync(cachePaths.documents, { recursive: true })

  const parserFingerprint = getParserFingerprint()
  const previousManifest = loadParseCacheManifest(cachePaths.manifest)
  const previousEntries =
    !force && previousManifest?.parserFingerprint === parserFingerprint
      ? previousManifest.entries
      : {}
  const fileEntries = files.map((file, index) => ({ file, index }))
  const fingerprints = await runWithConcurrency(
    fileEntries,
    concurrency,
    (item) => fingerprintFile(path.join(inputDir, item.file))
  )
  const nextEntries: Record<string, ParseCacheEntry> = {}
  let cacheHitCount = 0
  let parsedCount = 0
  ...
  await runWithConcurrency(
    ...
    async ({ file, index }) => {
      ...
      const cached = previousEntries[file]
      const hasCompleteCache =
        cached?.fingerprint === fingerprint &&
        cached.outputs.length > 0 &&
        cached.outputs.every((output) =>
          fs.existsSync(path.join(cachePaths.documents, output))
        )

      if (hasCompleteCache) {
        for (const output of cached.outputs) {
          materializeCachedFile(
            path.join(cachePaths.documents, output),
            path.join(outputDir, output)
          )
        }
        nextEntries[file] = cached
        cacheHitCount++
        return
      }
      ...
      try {
        ...
        for (const { outFile, contents } of rendered) {
          const cachePath = path.join(cachePaths.documents, outFile)
          fs.writeFileSync(cachePath, contents, "utf-8")
          materializeCachedFile(cachePath, path.join(outputDir, outFile))
          console.log(`  → ${outFile}`)
        }

        nextEntries[file] = {
          fingerprint,
          outputs: rendered.map((item) => item.outFile),
        }
        parsedCount++
      } catch (err) {
        failures.push(file)
  ...
  const referencedOutputs = new Set(
    Object.values(nextEntries).flatMap((entry) => entry.outputs)
  )
  for (const cachedFile of fs.readdirSync(cachePaths.documents)) {
    if (cachedFile.endsWith(".json") && !referencedOutputs.has(cachedFile)) {
      fs.unlinkSync(path.join(cachePaths.documents, cachedFile))
    }
  }
  writeManifestAtomically(cachePaths.manifest, {
    version: 1,
    parserFingerprint,
    entries: nextEntries,
  })
  ...
  console.log(
    `Parse cache: ${cacheHitCount} reused, ${parsedCount} parsed ` +
      `(concurrency ${concurrency}${force ? ", forced" : ""})`
  )
